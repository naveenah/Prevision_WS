...
def sanitize_ai_prompt(prompt: str) -> str:
    ...
    if len(prompt) > max_prompt_length:
        logger.info(f"Prompt truncated from {len(prompt)} to {max_prompt_length} characters")
...
def validate_file_upload(file, allowed_types: List[str], max_size_mb: int = 50) -> Dict[str, Any]:
    """
    ...
    if file_type in expected_extensions:
        if extension not in expected_extensions[file_type]:
            result['valid'] = False
            result['error'] = f'File extension {extension} does not match content type {file_type}'
            return result
